...
impl ChessMove {
    ...
    pub fn from_san(board: &Board, move_text: &str) -> Result<ChessMove, Error> {
        ...
        let promotion = if let Some(s) = move_text.get(cur_index..(cur_index + 1)) {
            match s {
                // Support both "Q" and "=Q" style promotion
                "=" => {
                    // Look ahead for the piece after '='
                    if let Some(piece_char) = move_text.get(cur_index + 1..cur_index + 2) {
                        cur_index += 2;
                        match piece_char {
                            "N" => Some(Piece::Knight),
                            "B" => Some(Piece::Bishop),
        ...
        // Automatically detect en passant if destination matches en passant square
        if !ep && takes && board.en_passant().is_some() && Some(dest) == board.en_passant() {
            ep = true;
        }
        ...
        for m in &mut MoveGen::new_legal(board) {
            ...
            if takes {
                // For en passant, the destination square is empty, but the move is still a capture
                let is_en_passant = ep
                    && board.en_passant().is_some()
                    && Some(m.get_dest()) == board.en_passant();
                if !is_en_passant && board.piece_on(m.get_dest()).is_none() {
                    continue;
                }
            }
