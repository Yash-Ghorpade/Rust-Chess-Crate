...
impl PawnType {
    ...
    pub fn legal_ep_move(board: &Board, source: Square, dest: Square) -> bool {
        let color = board.side_to_move();
        // The square of the pawn being captured en-passant. This is found by moving
        // one square "backward" from the destination square from the current player's perspective.
        let captured_pawn_sq = dest.ubackward(color);

        // Simulate the move by creating a temporary bitboard of all pieces.
        // We remove the capturing pawn from its source, remove the captured pawn,
        // and add the capturing pawn to the destination square.
        let combined = *board.combined()
...
impl PieceType for PawnType {
        ...
        T: CheckType,
    {
        ...
        if let Some(ep_sq) = board.en_passant() {
            ...
            // BUG FIX 1: The rank of the capturing pawn is one rank "behind" the en-passant square
            // from the current player's perspective. The original code incorrectly used `uforward`
            // for Black, which looked for a capturing pawn on the wrong rank. Using `ubackward`
            // is correct for both colors.
            let src_rank_square = ep_sq.ubackward(color);
            let src_rank = src_rank_square.get_rank();

            let files = get_adjacent_files(ep_sq.get_file());
            let src_bb = get_rank(src_rank) & files & pieces;
            
            let mut bb = src_bb;
            while bb != EMPTY {
                let src = bb.to_square();
                bb ^= BitBoard::from_square(src);
                let dest = ep_sq;
                // BUG FIX 2 is inside the corrected `legal_ep_move` function.
