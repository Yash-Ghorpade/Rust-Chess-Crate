...
/// None. The `dest` parameter is the destination square of the pawn that just made a double move.
fn set_ep(&mut self, ep_sq: Square, dest: Square) {
    // Only set self.en_passant if an opposing pawn is in the correct position to capture.
    // The opposing pawn must be on an adjacent file and on the same rank as the pawn that just moved.
    if get_adjacent_files(ep_sq.get_file())
        & get_rank(dest.get_rank()) // <-- FIX: Use the destination rank
    ...
    pub fn make_move_new(&self, m: ChessMove) -> Board {
        ...
        if moved == Piece::Knight {
            ...
            {
                // FIX: The en-passant square is the one the pawn *skipped*, not its destination.
                let ep_sq = dest.ubackward(self.side_to_move);
                result.set_ep(ep_sq, dest);
                result.checkers ^= get_pawn_attacks(ksq, !result.side_to_move, dest_bb);
            // FIX: An en-passant capture occurs when the pawn lands ON the en-passant square.
            } else if Some(dest) == self.en_passant {
                // FIX: The captured pawn is on the rank behind the destination square from the
                // opponent's perspective.
                let captured_pawn_sq = dest.ubackward(!self.side_to_move);
                result.xor(
                    Piece::Pawn,
                    BitBoard::from_square(captured_pawn_sq),
                    !self.side_to_move,
                );
                result.checkers ^= get_pawn_attacks(ksq, !result.side_to_move, dest_bb);
            } else {
...
impl TryFrom<&BoardBuilder> for Board {
    ...
    fn try_from(fen: &BoardBuilder) -> Result<Self, Self::Error> {
        ...
        if let Some(ep) = fen.get_en_passant() {
            // Temporarily switch to the perspective of the side that just moved.
            board.side_to_move = !board.side_to_move;

            // FIX: Calculate the destination square from the en-passant square.
            // The destination of the double-pawn-push is one square forward (from the
            // moving pawn's perspective) of the en-passant square.
            let dest = ep.uforward(board.side_to_move);
            board.set_ep(ep, dest);

            // Switch back to the correct side to move.
            board.side_to_move = !board.side_to_move;
        }
